...
### 기존 구현의 한계: FFmpeg 파이프라인의 구조적 결함과 I/O 병목

현재 업계 표준인 FFmpeg filter_complex 기반 멀티뷰 구현은 순차적 오버레이(Sequential Overlay) 방식을 따르며, 이는 HPC 환경에서 다음과 같은 치명적인 아키텍처 한계를 가진다.

첫째, 직렬화된 메모리 접근으로 인한 대역폭 낭비 스트림 개수 N에 대해 N번의 오버레이 필터가 직렬로 연결되며, 각 단계마다 불필요한 Global Memory Read/Write가 반복된다. 이는 연산 능력보다 메모리 대역폭(Memory Bandwidth)이 성능을 좌우하는 GPU 환경에서 VRAM 대역폭을 급격히 고갈시키는 주원인이다.

둘째, 커널 실행 오버헤드의 누적 각 필터 단계마다 개별 CUDA 커널이 독립적으로 실행·종료된다. 이로 인해 실제 픽셀 연산 비용보다 커널 런치(Kernel Launch) 및 동기화 오버헤드가 지배적인 비용으로 작용하여 GPU 가동률(Occupancy)을 저하시킨다.

셋째, 이기종 필터 혼용으로 인한 제로 카피(Zero-Copy) 원칙 붕괴 정교한 커스텀 레이아웃이나 특수 효과 구현 시, FFmpeg의 GPU 미지원(CPU-only) 필터가 필수적으로 요구되는 경우가 발생한다. 이 경우 강제적인 hwdownload/hwupload 과정이 개입되어 데이터가 VRAM과 Host Memory(CPU)를 왕복하게 된다. 이는 PCIe 대역폭을 심각하게 소모할 뿐만 아니라, CPU-GPU 간 동기화 대기로 인해 전체 파이프라인의 처리량을 최저 성능 구간(Bottleneck)인 CPU 속도에 맞추게 만든다.

넷째, 선형적 깊이 증가에 따른 레이턴시 폭증 스트림 수 증가에 따라 파이프라인 깊이(Depth)가 선형적으로 증가하며, UHD 고해상도 환경에서는 레이턴시가 기하급수적으로 늘어나는 구조적 한계를 보인다.

결론 본 팀은 선행 연구로 4채널 UHD 멀티뷰 프로토타입을 구현하였으나, 스트림 확장 시 프레임 드랍과 급격한 지연을 관측하였다. 이는 단순 파라미터 튜닝으로 해결 불가능하며, **FFmpeg 필터 체인의 직렬성 및 이기종 간 데이터 이동(Data Movement)** 에서 기인한 아키텍처 레벨의 근본적 병목임으로 판단하였다.
...
<img width="1024" height="559" alt="image" src="https://github.com/user-attachments/assets/402f3a63-84f5-46c6-b122-e6c184014596" />
